...
## PURPOSE
Extend the MCRN AI plugin with SAFE, READ-ONLY tools. The core contract remains: one raw shell line per response. Pipes, redirects, and logical operators (&&, ||, ;) within that single line are permitted. Multi-line output (backslash continuations, heredocs) is forbidden.
