...
3. Create ralph tasks using OpenCode's native `todowrite` tool
...
### Then, create the subtasks with parentID and dependsOn:
...
- Query `todoread()` with `repoURL: "<repo-url>"` (no ready filter)
...
6. Mark task as completed using todowrite to update status to "completed"
...
```
- Validate  the a11y tree as text that can be read and verified
